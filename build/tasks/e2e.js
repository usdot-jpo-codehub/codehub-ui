...
gulp.task('webdriver-update', function(done){
   webdriverUpdate(done);
});

gulp.task('webdriver-standalone', function(done){
  gulp.series(['webdriver-update'], webdriverStandalone)(done);
});

gulp.task('clean-e2e', function(done) {
  del(paths.e2eSpecsDist + '*');
  done();
});
...
gulp.task('build-e2e', function(done){
  gulp.series(['clean-e2e'], function() {
...
gulp.task('e2e', function(done){
  gulp.series(['build-e2e'], function(cb) {
